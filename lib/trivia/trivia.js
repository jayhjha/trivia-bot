...
const scheduleNextQuestion = async ({ app }) => {
  ...
  if (!nextQuestionAnswer) {
    endTrivia({ app, channel: channelId });
    return;
  }

  logger.info('Scheduling next question...');
  await postMessage({ app, channel: channelId, text: `Next question in ${INTERVAL} seconds` });
...
const handleTriviaAnswer = async ({
  app, channel, text, user,
}) => {
  if (triviaState.isActive && verifyAnswer({ text })) {
    await postMessage({ app, channel, text: `${text} is the correct answer :tada: \n Well done, <@${user}|cal>` });
